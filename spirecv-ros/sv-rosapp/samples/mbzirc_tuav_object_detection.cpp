...
{
  	ros::init(argc, argv, "mbzirc_tuav_object_detection");
  	ros::NodeHandle nh("~");
	...
	cod_init.loadAlgorithmParams(sv::get_home() + "/spirecv-ros/sv-rosapp/algorithm_params/sv_algorithm_params_mbzirc_1280.json");
	cod_sub_region.loadAlgorithmParams(sv::get_home() + "/spirecv-ros/sv-rosapp/algorithm_params/sv_algorithm_params_mbzirc_640.json");
	...
	afod.loadAlgorithmParams(sv::get_home() + "/spirecv-ros/sv-rosapp/algorithm_params/sv_algorithm_params_tuav.json");
	...
	while (ros::ok())
	{
		if (get_image == true)
		{
			...
			spirecv_msg_pub.publish(ros_tgts);
                       //streamer.stream(img);
			cv::namedWindow("tuav_object_det", cv::WINDOW_KEEPRATIO);
			cv::resizeWindow("tuav_object_det", 640, 360);
			cv::imshow("tuav_object_det", img);
			cv::waitKey(1);
		}
		ros::spinOnce();
	}
